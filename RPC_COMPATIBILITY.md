...
Wren serves JSON-RPC at `http://127.0.0.1:1248` and `ws://127.0.0.1:1248`. Legacy HTTP origins and WebSocket metadata are client assertions, not process identities. Native clients can instead use the originless authenticated protocol-v3 routes described below. Valid web origins are full, scheme-preserving URIs. Other originless, opaque, malformed, oversized, and schemeless clients receive a server-generated identity scoped to their connection; see [Threat Model](THREAT_MODEL.md).
