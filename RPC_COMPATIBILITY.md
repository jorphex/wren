...
| Chains       | `wallet_addEthereumChain`, `wallet_switchEthereumChain`, `wallet_getEthereumChains`                          | Adds a chain after metadata validation and confirmation. An authorized origin can switch its own route to an enabled chain without another approval. Before account access, a canonical web origin can request a separate network-change approval that names the site and target chain without exposing an account; account access still requires its own consent. Untouched requests on the old route are cancelled. The non-standard getter returns enabled Wren chains.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
- Companion protocol v3 mutually authenticates Wren and an atomic control/page key bundle with signed, role-bound, expiring transcripts and an explicit six-digit comparison. Companion 0.1.1 omits browser/runtime UUID fields; Wren validates the browser-supplied WebSocket Origin transiently and persists only signed installation/key identity. Legacy v3 hellos are accepted only when both transport fields exactly match that Origin, then the fields are discarded. Page sessions cannot pair. Rotation is acknowledgement-safe. Revocation removes source-bound grants, requests, subscriptions, and transports. Protocol-v2 clients must upgrade and cannot downgrade silently.
