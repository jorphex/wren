...
| Assets       | `wallet_watchAsset`, `wallet_getAssets`                                                                      | Supports legacy ERC-20 and bounded ERC-1046 ERC-20 suggestions, plus a non-standard selected-account asset getter. The getter does not prompt and returns `4100` before permission. ERC-1046 accepts inline JSON and IPFS metadata only. It does not fetch HTTP(S) metadata or images.                                                                                                                                                                                                                 |
| Wallet calls | `wallet_sendCalls`, `wallet_getCallsStatus`, `wallet_showCallsStatus`, `wallet_getCapabilities`              | Supports EIP-5792 `2.0.0` non-atomic sequential calls only. Status is persisted and scoped to origin/account. `atomic.status` is `"unsupported"`. Signed or submitted batches resume evidence-only receipt and finality checks after restart and are never automatically rebroadcast.                                                                                                                                                                                                                  |
