...
| Permissions    | `wallet_getPermissions`, `wallet_requestPermissions`                                                         | EIP-2255 `eth_accounts` only. A bounded `requiredMethods` hint is checked against the selected signer before prompting; it is not a persisted grant caveat.                                                                                                                                                |
...
Account, signing, transaction, chain mutation, asset, wallet-call, and legacy-envelope methods require origin permission. Passive account, asset, and capability probes do not open UI; they return empty results or `4100` without permission. `wallet_requestPermissions` is callable before authorization so it can prompt; `wallet_getPermissions` returns an empty list when absent.
...
- A same-user malicious process can assert browser-like origin metadata; permission is not process identity.
