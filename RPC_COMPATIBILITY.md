...
Wren serves browser-compatible JSON-RPC at `http://127.0.0.1:1248/` and `ws://127.0.0.1:1248/`. These root routes require a canonical HTTP(S), WS(S), or browser-extension `Origin`; the asserted origin is compatibility metadata, not process authentication. Originless native and CLI clients must use the authenticated protocol-3 routes described below. Unknown routes, malformed origins, reserved internal labels, and protocol-version mismatches fail before provider dispatch or consent UI; see [Threat Model](THREAT_MODEL.md).
...
| Wallet calls | `wallet_sendCalls`, `wallet_getCallsStatus`, `wallet_showCallsStatus`, `wallet_getCapabilities`              | EIP-5792 `2.0.0`, non-atomic sequential calls only. Status is persisted and scoped to origin/account; `atomic.status` is `"unsupported"`. Signed/submitted batches resume evidence-only receipt and finality checks after restart and are never automatically rebroadcast.                                       |
...
- Originless, opaque, malformed, schemeless, and reserved-origin root clients receive `4100` and must pair through native protocol 3. They cannot create a permission identity or open access UI.
