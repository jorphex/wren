...
| [EIP-747](https://eips.ethereum.org/EIPS/eip-747)   | Implemented | `wallet_watchAsset` supports the standard's checksummed legacy ERC-20 and ERC-1046 suggestions on connected chains and acknowledges valid requests before prompting. ERC-1046 metadata is bounded to inline JSON or IPFS through Wren's configured gateway or explicit Kubo endpoint, reconciled with contract reads, and validated before acknowledgement; arbitrary HTTP(S) metadata and metadata images are rejected. Other asset types require separate standards.                          |
