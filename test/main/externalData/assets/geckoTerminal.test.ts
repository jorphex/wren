...
test('resumes remaining tokens after a rate-limited pool search', async () => {
  const next = pool(80, 50000, '7')
  next.relationships.base_token.data.id = `eth_${other}`
  const fetchImpl = jest
    .fn()
    .mockResolvedValueOnce(response(Array.from({ length: 20 }, (_, i) => pool(i + 10))))
  ...
  expect(await finish(pending)).toEqual({ [identifier]: { price: 2 }, [`ethereum:${other}`]: { price: 7 } })
})

test('cancels the remaining token queue during provider backoff', async () => {
  const fetchImpl = jest.fn(async () => response([], 429))
  const controller = new AbortController()
  const pending = createGeckoTerminalPrices(fetchImpl)([identifier, `ethereum:${other}`], controller.signal)
